...
function createMessageQueue(config: QueueConfig) {
  ...
  }, callback: (body: string, from: string) => Promise<void>): Promise<void> {
    ...
    userQueue.timer = setTimeout(async () => {
      ...
      if (currentQueue?.callback) {
        ...
        try {
          ...
          console.log("Processing queue for", from, ":", result);
          ...
          if (currentQueue.messages.length > 0) {
            console.log(
              "New messages arrived during processing, restarting timer",
            );
            currentQueue.timer = setTimeout(async () => {
              ...
              if (
                ...
                try {
                  ...
                  console.log(
                    "Processing remaining queue for",
                    from,
                    ":",
                    result,
                  );
                  ...
                  console.error(
                    "Error processing remaining queue for",
                    from,
                    ":",
                    error,
                  );
